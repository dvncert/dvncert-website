...
/**
 * Eğitim ön kayıt / ilgi formu. Belirli bir eğitim sayfasına gömülür; talep
 * admin panelindeki "Form Gönderileri" gelen kutusuna düşer. Eğitimler genel
 * katılıma açıktır (kurum bazlı eğitim yapılmaz).
 */
export default function EgitimKayitFormu({ egitimAdi, egitimSlug }: { egitimAdi: string; egitimSlug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const tercih = String(data.get("tercih") ?? "");
    ...
    const sonuc = await formGonderAction({
      ...
      konu: `Eğitim ön kayıt: ${egitimAdi}`,
      mesaj: `${String(data.get("mesaj") ?? "")}\n\nKatılım tercihi: ${tercih}`.trim(),
      ekVeri: { egitim: egitimAdi, slug: egitimSlug, tercih },
  ...
  return (
    <form onSubmit={handleSubmit} className="dvn-iletisim-form" style={{ display: "grid", gap: 14, position: "relative" }}>
      <SpamKorumasi />
      <div className="dvn-form-satir" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      ...
      <div className="dvn-form-satir" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <label style={alanStili}>
          <span style={etiketStili}>Katılım tercihi</span>
          <select name="tercih" style={inputStili} defaultValue="Farketmez">
            <option>Çevrim içi (online)</option>
      ...
      <label style={alanStili}>
        <span style={etiketStili}>Notunuz</span>
        <textarea name="mesaj" rows={3} placeholder="Eklemek istedikleriniz (opsiyonel)" style={{ ...inputStili, resize: "vertical", minHeight: 80 }} />
      </label>
      ...
      <label style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 12.5, color: "var(--dvn-gri-500)", lineHeight: 1.5 }}>
        ...
        <span>
          <Link href="/kvkk" style={{ color: "var(--dvn-turuncu)", fontWeight: 500 }}>KVKK Aydınlatma Metni</Link>’ni okudum ve kişisel verilerimin işlenmesini kabul ediyorum.
        </span>
      </label>
      ...
      >
        {gonderiliyor ? "Gönderiliyor..." : "Ön Kayıt / Bilgi Talebi"}
      ...
      {gonderildi && (
        <p style={{ fontSize: 13, color: "var(--dvn-altin)", margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          Talebiniz alındı. Eğitim tarihleri ve kayıt için sizinle iletişime geçeceğiz.
        </p>
      )}
      {hata && (
      ...
      <style>{`
        ...
        .dvn-iletisim-form select:focus,
        ...
        .dvn-iletisim-form textarea::placeholder { color: var(--dvn-gri-300); }
