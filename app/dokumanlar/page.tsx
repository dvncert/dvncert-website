...
import { dokumanGruplari, type DokumanTipi } from "@/lib/dokumanlar";
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${siteConfig.url}/dokumanlar` },
};

const tipRenkleri: Record<DokumanTipi, { arka: string; metin: string }> = {
  PDF: { arka: "#fdecea", metin: "#d93025" },
  DOCX: { arka: "#e8f0fe", metin: "#1a73e8" },
  XLSX: { arka: "#e6f4ea", metin: "#188038" },
};

export default function DokumanlarSayfasi() {
  return (
    <main>
      <script
      ...
      <section style={{ background: "white", padding: "60px 32px" }}>
        <div style={{ maxWidth: 920, margin: "0 auto" }}>
          ...
          {dokumanGruplari.map((grup) => (
            <div key={grup.grup} style={{ marginBottom: 36 }}>
              <h2
                style={{
                  fontSize: 13,
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {grup.dokumanlar.map((d) => {
                  const renk = tipRenkleri[d.tip];
                  return (
                    <div
                      key={d.kod + d.baslik}
